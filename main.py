...
from config import MT5_CONFIG, DUAL_SOURCE_ASSETS, YF_SYMBOLS, MACRO_WEIGHTS, SIGNAL_CONFIG
...
def print_score(score_result: dict, delta_result: dict = None):
    ...
    print(f"  AÇÃO: {signal.get('action', 'N/A')}")
    ...
    if delta_result:
        ...
        print(f"  CONFIANÇA: {entry.get('confidence', 'N/A')}")
    
    print(f"{'─' * 70}")


def print_categories(score_result: dict):
    """Imprime breakdown por categoria."""
...
def print_assets(score_result: dict):
    """Imprime detalhes por ativo."""
    asset_signals = score_result.get("asset_signals", {})
    
    print(f"\n  📋 ATIVOS:")
    print(f"    {'Ativo':15s} {'Preço':>12s} {'Variação':>10s} {'Direção':>10s} {'Contrib':>10s} {'Fonte':>6s}")
    print(f"    {'─' * 65}")
...
def main():
    """Função principal do sistema terminal."""
    print_header()
    ...
    print(f"\n  🔄 Atualização automática a cada {interval}s. Ctrl+C para sair.\n")
